...
size_t utils::cubTempBytes(int maxItems)
{
	...
	cub::DeviceScan::ExclusiveSum(nullptr, bytes, i1, i1, maxItems);
	maxBytes = std::max(maxBytes, bytes);

	bytes = 0;
	cub::DeviceRadixSort::SortPairs(nullptr, bytes, i1, i1, i1, i1, maxItems);
	maxBytes = std::max(maxBytes, bytes);

	bytes = 0;
	cub::DeviceRadixSort::SortPairs(nullptr, bytes, i1, i1, i2, i2, maxItems);
	maxBytes = std::max(maxBytes, bytes);

	bytes = 0;
	cub::DeviceRadixSort::SortPairs(nullptr, bytes, i1, i1, i3, i3, maxItems);
	maxBytes = std::max(maxBytes, bytes);

	bytes = 0;
	cub::DeviceMergeSort::SortPairs(nullptr, bytes, i2, i1, maxItems, CustomLess());
	maxBytes = std::max(maxBytes, bytes);

	bytes = 0;
	cub::DeviceReduce::Min(nullptr, bytes, i1, i1, maxItems);
	maxBytes = std::max(maxBytes, bytes);

	bytes = 0;
	cub::DeviceReduce::Max(nullptr, bytes, i1, i1, maxItems);
	maxBytes = std::max(maxBytes, bytes);

	// Safety margin: keep room for growth between the query and the call.
	return maxBytes + maxBytes / 4 + (1u << 20);
}
...
RKERNEL(int*, int*, int*, int) //reorderIndex_s
