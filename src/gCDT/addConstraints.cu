...
double addConstraints(void* cubTempStorage, size_t cubTempStorageBytes, Point2d* points, int3* tris, int3* adjTris, int* sons, int* bitmapOffsets, int* bitmapTris, int2* cons, int3*& newTris, int numCons, int numPoints, int numTriangles, int& numNewTris)
{
	...
	// A degenerate strip walk can emit the same polygon vertex twice (it walks
	// out to a vertex and back). The distance-ordered triangulation below cannot
	// resolve two copies of one vertex: they compare equal in every key and both
	// end up falling back to the constraint endpoints, which produces two
	// triangles sharing the constraint edge on the same side (non-manifold).
	// Drop the repeated occurrence, which leaves the walk's simple path.
	{
		std::vector<int> h_p, h_c;
		h_p.resize(numPolyP);
		h_c.resize(numPolyP);
		utils::memcpy(h_p.data(), polyP, numPolyP, cudaMemcpyDeviceToHost);
		utils::memcpy(h_c.data(), polyC, numPolyP, cudaMemcpyDeviceToHost);

		std::vector<int> h_p2, h_c2;
		h_p2.reserve(numPolyP);
		h_c2.reserve(numPolyP);
		std::unordered_set<int> seen;
		int run = 0;
		for (int i = 0; i < numPolyP; ++i) {
			if (i == 0 || h_c[i] != h_c[i - 1]) {
				seen.clear();
				++run;
			}
			if (!seen.insert(h_p[i]).second) continue;  // repeated vertex in this polygon
			h_p2.push_back(h_p[i]);
			h_c2.push_back(h_c[i]);
		}
		if ((int)h_p2.size() != numPolyP) {
			utils::release(polyP, numPolyP);
			utils::release(polyC, numPolyP);
			numPolyP = (int)h_p2.size();
			utils::malloc(polyP, numPolyP);
			utils::malloc(polyC, numPolyP);
			utils::memcpy(polyP, h_p2.data(), numPolyP, cudaMemcpyHostToDevice);
			utils::memcpy(polyC, h_c2.data(), numPolyP, cudaMemcpyHostToDevice);
		}
	}
	...
	// Debug dump of the polygon construction and its retriangulation, used to
	// analyse degenerate constraint configurations. Enabled with
