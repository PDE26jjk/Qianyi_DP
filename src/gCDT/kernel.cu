...
// Carries a caller-owned index array (original vertex ids) through the same
// permutation that reorderPoint_s2 applies to the vertex array.
__global__ void reorderIndex_s(int* index, int* indexSorted, int* labelsSorted, int numP)
{
	int tid = threadIdx.x + blockIdx.x * blockDim.x;

	if (tid >= numP) return;

	indexSorted[tid] = index[labelsSorted[tid]];
}
...
double reorderPoints(void* cubTemp, size_t cubBytes, Point2d*& points, int3* tris, int2* cons, int numPoints, int numTriangles, int numCons, int* origIndex = nullptr)
{
	int* codes;
	int* labels;
	int* codesSorted, * labelsSorted;
	...
	if (origIndex != nullptr) {
		int* indexSorted;
		utils::malloc(indexSorted, numPoints);
		utils::kernel(reorderIndex_s, numPoints, 128, origIndex, indexSorted, labelsSorted, numPoints);
		utils::memcpy(origIndex, indexSorted, numPoints, cudaMemcpyDeviceToDevice);
		utils::release(indexSorted, numPoints);
	}
...
double delaunayNew(void* cubTemp, size_t cubBytes, Point2d*& points, int3*& tris, int2* cons, int numPoints, int numTriangles, int numCons, int* origIndex = nullptr)
{
	double costTime = 0;
	costTime += reorderPoints(cubTemp, cubBytes, points, tris, cons, numPoints, numTriangles, numCons, origIndex);
