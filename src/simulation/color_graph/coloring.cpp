...
struct Graph {
    Graph(int num_nodes_in, const std::vector<int2>& edges)
...
int graph_coloring_degree_ordered_greedy(Graph& graph) {
    ...
    std::ranges::sort(ordering, [&degrees](const auto& lhs, const auto& rhs) {
...
int graph_coloring_cpu(int num_nodes, const std::vector<int2>& edges, int algorithm, std::vector<int>& node_colors, bool balance,
    ...
    case 1:
        ...
        break;
    default:
        throw std::runtime_error("Unrecognized coloring algorithm number");
    }
