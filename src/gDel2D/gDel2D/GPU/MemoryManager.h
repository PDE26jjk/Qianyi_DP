...
public: 
    ...
    DevVector( ) : _ptr(), _size( 0 ), _capacity( 0 ), _owned( false ) {}
    
    DevVector( size_t n ) : _ptr(), _size( 0 ), _capacity( 0 ), _owned( false )
    {
        resize( n );
        return;
    }

    DevVector( size_t n, T value ) : _ptr(), _size( 0 ), _capacity( 0 ), _owned( false )
    {
        assign( n, value );
        return;
    }
    ...
    void free()
    {
        if ( _capacity > 0 && _owned && _ptr.get() != nullptr )
            CudaSafeCall( cudaFree( _ptr.get() ) );

        _size       = 0;
        _capacity   = 0;
        _ptr        = thrust::device_ptr<T>();  // Reset to null pointer
    ...
    void resize( size_t n )
    {
        ...
        try
        {
            // Use cudaMalloc instead of deprecated thrust::device_malloc
            T* raw_ptr = nullptr;
            cudaError_t err = cudaMalloc(&raw_ptr, _capacity * sizeof(T));
            if (err != cudaSuccess) {
                throw std::runtime_error("cudaMalloc failed");
            }
            _ptr = thrust::device_ptr<T>(raw_ptr);
        }
        catch( ... )
        {
